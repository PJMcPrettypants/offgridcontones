...
import { intersect } from "./lineIntersect.js";
import { polygonArea, polygonContains } from "d3-polygon";
import { naturalNeighborInterpolate } from "./naturalNeighborInterpolate.js";
...
const sketch = (p) => {
  ...
  function createNNInterpolation(strideVP) {
    ...
    for (
      let y = renderNNCounter;
      y < stepEnd - 1 && y < p.height;
      y += strideVP
    ) {
      for (let x = 0; x < p.width; x += strideVP) {
        let NNweightedColor = naturalNeighborInterpolate(
          x,
          y,
          colPoints,
          points,
  ...
  function circumcenterSubdivision() {
      ...
      if (triNotOutOfInnerBounds(point1, point2, point3)) {
        ...
        if (!polygonContains(ccTriPoly, [cxLonger, cxLonger])) {
          ...
          if (
            ...
            let subdividedColor = naturalNeighborInterpolate(
              cxShort,
              cyShort,
              colPoints,
              points,
    ...
    for (let i = 0; i < subdivColPoints.length; i++) {
      if (pointWontBeDuplicate(colPoints, subdivColPoints[i]))
    ...
    while (currentHighRatio > radiusEdgeRatioLimit) {
      ...
      console.log(
        "triToSubdivide:" +
          triToSubdivide +
          " of " +
          radiusEdgeRatios.length +
          " with ratio: " +
          radiusEdgeRatios[triToSubdivide]
      );
  ...
  function pointsFromImage(sampleProbability, pointThreshold, sampleJitter) {
    ...
    linearImage = makeImageLinear(
      copiedImage,
      sourceImage.width,
      sourceImage.height
    );

    colPoints = findPoints(
      linearImage,
      sourceImage.width,
      sourceImage.height,
      sampleProbability,
  ...
  function pickPointFromImage(floatX, floatY) {
    sourceImage.loadPixels(); //is this needed?

    const x = parseInt(floatX);
    const y = parseInt(floatY);
  ...
  function parseLoadedPoints(jsonPoints) {
    ...
    for (let jp in as) {
      ...
      parsedColPoint[0] =
        (parsedColPoint[0] - bounds[2] / 20) * zoomFactorOnLoad +
        bounds[2] / 20;
      parsedColPoint[1] =
        (parsedColPoint[1] - bounds[2] / 20) * zoomFactorOnLoad +
        bounds[2] / 20;
      ...
      if (
        ...
        if (pointWontBeDuplicate(colPoints, parsedColPoint))
          colPoints.push(parsedColPoint);
...
function pointWontBeDuplicate(colPointArray, newColPoint) {
  for (const previousPoint of colPointArray) {
    if (
      previousPoint[0] == newColPoint[0] &&
      previousPoint[1] == newColPoint[1]
    ) {
      return false;
    }
  }
